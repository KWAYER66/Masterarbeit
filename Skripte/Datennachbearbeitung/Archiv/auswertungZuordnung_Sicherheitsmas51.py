...
kl_divtest2= np.zeros(51)
kl_divtest2[1] = 1
data = np.load('D:\Masterarbeit\Daten\Input_NeuronalesNetz\AlignmentsPWMs_Zuordnung_Weights_Abst_tf.npy')[:,(6804+176):(6804+176+51)]
maxabweichung = float(kl_divergence_by_christian(data[0,:],kl_divtest).numpy())
print(maxabweichung)
maxabweichung = float(kl_divergence_by_christian(kl_divtest2,kl_divtest).numpy())
print(maxabweichung)
kl_divtest3= np.zeros(51)
kl_divtest3[10] = 1
maxabweichung = float(kl_divergence_by_christian(kl_divtest3,kl_divtest).numpy())
print(maxabweichung)
        ...
        for file in predictionordnerverzeichnis:
            if 'zuordnung' in file:
                ...
                for i in range(0, zuordnungsfile.shape[0]):
                    ausgabesicherheit.write(str(maxabweichung - float(kl_divergence_by_christian(data[i,:],zuordnungsfile[i,:]).numpy())/maxabweichung)+'\n')
                    sicherheit = sicherheit + (maxabweichung-float(kl_divergence_by_christian(data[i,:],zuordnungsfile[i,:]).numpy()))/maxabweichung
                ...
                for i in range(0, zuordnungsfile2.shape[0]):
                    ausgabesicherheit.write(str(maxabweichung - float(kl_divergence_by_christian(data2[i,:],zuordnungsfile2[i,:]).numpy())/maxabweichung)+'\n')
                    sicherheit = sicherheit + float(kl_divergence_by_christian(data2[i,:],zuordnungsfile2[i,:]).numpy())/maxabweichung
