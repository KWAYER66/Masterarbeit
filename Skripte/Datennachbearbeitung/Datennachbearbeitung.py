		...
		if 'gegenzufallsliste' in lists:
			...
			for file in predictionsneuronalenetze:
				if 'prediction' in file:
					kl_div = open(x+'\\'+file+'\\kl_divergencen2.tsv')
					kl_divGegen = open(x+'\\'+file+'\\kl_divergencenGegenListe2.tsv', 'w')
					kl_divGegenPlot = open(x+'\\'+file+'\\kl_divergencenGegenListePlot2.tsv', 'w')
					kl_divZ = kl_div.readlines()
					for i in range(0,len(kl_divZ)):
...
#erzeugegegenteilliste()
	#Soll auswertungsgegenliste oder nicht ?!
