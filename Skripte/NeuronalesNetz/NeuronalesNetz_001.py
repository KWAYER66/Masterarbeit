...
if sys.argv[1] == '176':
	datapfad = 'D:\Masterarbeit\Daten\Input_NeuronalesNetz\AlignmentsPWMs_Zuordnung_Weights.npy'
elif sys.argv[1] == '140':
...
def train_model(X,Y):
    ...
    model1.compile(loss=kl_divergence_by_christian,optimizer="adam")
    hist = model1.fit(X,Y,epochs=int(sys.argv[5]),batch_size=150,callbacks=[csv_logger])
    return model1
