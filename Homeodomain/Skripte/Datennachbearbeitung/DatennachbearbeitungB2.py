...
def Berechne_KL_Divergence():
	...
	for datei in pfadpredpwms:
		if ('prediction' in datei) & ('176' in datei):
			tsvdateimitdivergencen = open('D:\Masterarbeit\Predictions'+'\\'+datei+'\kl_divergencen3.tsv','w')
			for file in listpwmsnamen:
				if 'M00260_2.00' in file:
					...
					#tsvdateimitdivergencen.write(str(altpwm))
					tsvdateimitdivergencen.write('\n')
					tsvdateimitdivergencen.write(str(prepwm))
			...
			for file in listpwmsnamenG:
				if 'pwm' in file:
					...
					tsvdateimitdivergencen.write(file.split('.')[0]+'.'+file.split('.')[1]+'\t')
					tsvdateimitdivergencen.write(str(np.asarray(x[0,]))+'\n')
def Berechne_KL_Divergence2():
	pfadpredpwms = os.listdir('D:\Masterarbeit\Predictions')
	for datei in pfadpredpwms:
		if ('prediction' in datei) & ('176' in datei):
			tsvdateimitdivergencen = open('D:\Masterarbeit\Predictions'+'\\'+datei+'\kl_divergencen4.tsv','w')
			...
			for i in range(8,9):
				...
				x = kl_divergence_by_christian(altpwm,pred)
				tsvdateimitdivergencen.write(str(np.asarray(x[0,]))+'\n')
				#tsvdateimitdivergencen.write(str(altpwm))
				tsvdateimitdivergencen.write('\n')
				tsvdateimitdivergencen.write(str(prepwm))
