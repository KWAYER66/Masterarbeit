...
def MergeAlignmentsPwmZuordnungWeights():
	#ALLE
	pwmsgruppen = os.listdir('D:\Masterarbeit\Daten\PWMS_Nach_Gruppen_ueberarbeitetmitalle')
	...
	for file in alignments:
		if ('aln' in file):
			...
			for potentiellrepresentant in representativliste:
				...
				#print(representanttfaktor)
				if (len(representanttfaktor) != 0) & ('ergebnis' in potentiellrepresentant):
					represente = os.listdir('D:\Masterarbeit\DBDs\\'+potentiellrepresentant+'\\Motifs_representative')
					for represent in represente:
						if 'Full' in represent:
							if file.split('.')[0] == represent.split('_')[1]+'_2':
								print(potentiellrepresentant)
								print(represent)
								wievielerepre = wievielerepre + 1
								weightarr[0,0] = 1
				elif (len(representanttfaktor) == 1) & ('ergebnis' not in potentiellrepresentant):
					if file.split('.')[0] == representanttfaktor[0].split('.')[0]:
						print(potentiellrepresentant)
						print(file)
						weightarr[0,0] = 1
						wievielerepre = wievielerepre + 1
			if weightarr[0,0] == 0:
				wievielerepre2 = wievielerepre2 +1
			myarray = np.append(myarray,weightarr,axis = 1)
			arr = np.append(arr, myarray, axis =0)
	print(arr.shape)
	print(wievielerepre)
	print(wievielerepre2)
