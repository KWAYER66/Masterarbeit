...
def train_model(Z,Y,weights):
	input_layer = Input(shape = (X.shape[1],X.shape[2]), name='DBDs')
	input_layer2 = Input(shape= (X2.shape[1],X2.shape[2]),name='paarweiseAehnlichkeiten')
	input_layer3 = Input(shape= (X3.shape[1],X3.shape[3]),name='zuordnung')

	initializer = tf.keras.initializers.GlorotUniform()

	LSTM_1_1 = LSTM((176), activation="relu", input_shape=(None,1,6804), kernel_initializer=initializer)(input_layer)
	Dropout_1_1 = Dropout(0.25)(LSTM_1_1)
	dense1_2 = Dense((704),activation="relu")(Dropout_1_1)
	dense1_3 = Dense((1408),activation="relu")(dense1_2)
	flat1 = Flatten()(dense1_3)

	LSTM_2_1 = LSTM((500), activation="relu", input_shape=(None,1,1618), kernel_initializer=initializer)(input_layer2)
	Dropout_2_1 = Dropout(0.25)(LSTM_2_1)
	dense_2_2 = Dense((102), activation="relu")(Dropout_2_1)
	flat2 = Flatten()(dense_2_2)

	LSTM_3_1 = LSTM((102), activation="relu", input_shape=(None,1,51), kernel_initializer=initializer)(input_layer3)
	Dropout_3_1 = Dropout(0.25)(LSTM_3_1)
	dense_3_2 = Dense((153), activation="relu")(Dropout_3_1)
	flat3 = Flatten()(dense_3_2)
	...
	model = Model(inputs = [input_layer,input_layer2,input_layer3], outputs = outputs1, name = 'functional_model2')
	model.compile(loss = kl_divergence_by_christian, optimizer = 'adam')

	model.fit(Z,Y, epochs = int(sys.argv[5]), batch_size = 150, sample_weight=weights, callbacks=[csv_logger])
	return model
...
TestD = tf.reshape(TestD,shape=[-1,1,6804])
TestD2 = tf.reshape(TestD2,shape=[-1,1,1618])
TestD3 = tf.reshape(TestD3,shape=[-1,1,51])
Z2 = {
	'DBDs': TestD,
