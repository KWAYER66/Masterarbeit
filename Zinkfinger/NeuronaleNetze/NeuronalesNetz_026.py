...
from tensorflow.keras.layers import MaxPooling2D, Conv2D, Dense, Activation, Dropout, Conv1D, Flatten, MaxPooling1D, GlobalAveragePooling1D,Reshape, LSTM, Input, Lambda, Concatenate
import sys
import os
...
def train_model(X,Y):
	...
	Conv1_1 = Conv1D(filters =30, kernel_size=1, strides = 1,activation="relu", input_shape=[X.shape[1],X.shape[2]])(input_layer)
	lstm = LSTM(30, return_sequences = True)(Conv1_1)
	flatten1 = Flatten()(lstm)
	dense1_1 = Dense((2500),activation="relu")(flatten1)
	dense1_2 = Dense((4000),activation="relu")(dense1_1)
	dense1_3 = Dense((1000),activation="relu")(dense1_2)

	outputs1 = Dense((200), activation = softmax_by_christian, name = 'pwm')(dense1_3)
...
#Z = Conv1D(filters = 200, kernel_size=27, strides = 1188,activation="relu", input_shape=[X.shape[0],X.shape[1],X.shape[2]])(X)
